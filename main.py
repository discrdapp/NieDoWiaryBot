...
token = os.environ['token']  # add your own token
# emojis
emoji_worked = "✅"
emoji_error = "❌"
...
async def get_user_input(message, default_spell=True):
	...
	answer = await client.wait_for(
	    "message",
	    check=lambda response: response.author == message.author and response.
...
async def get_user_id(param):
	reception_user_beta = str(
	    param[1])  # the mention in channel gives us <@!USERID> OR <@USERIRD>
	reception_user = ""
...
async def get_role_id_single(parameter):
	role_beta = str(
	    parameter)  # see another instance where i use this to see why
	role_clean = ""
...
async def send_error(channel):
	embed = discord.Embed(title="Error.",
	                      description="Internal Error, call admin.",
	                      color=0xff0000)
	await channel.send(embed=embed)
	return
...
async def on_ready():
	...
	if check_status == "error":
		...
		embed = discord.Embed(
		    ...
		    color=color)
		embed.set_author(
		    name="UnbelievaBoat-Python Bot",
		    icon_url=
		    "https://blog.learningtree.com/wp-content/uploads/2017/01/error-handling.jpg"
		)
		embed.set_footer(
		    text=
		    "tip: default config at https://github.com/NoNameSpecified/UnbelievaBoat-Python-Bot"
		)
		# await channel.send(embed=embed)
		quit()
...
async def on_message(message):
	...
	try:
		for test_cmd in range(len(command)):
			if command[test_cmd].startswith('"') or command[test_cmd].startswith(
				...
				new_slide = ""
				temp_cmd = test_cmd
				while not (command[temp_cmd].endswith('"')
				           or command[temp_cmd].endswith("'")):
		...
		await message.channel.send(
		    "Error. You maybe opened a single/doublequote or a < and didnt close it"
		)
	...
	if command in ["blackjack", all_reg_commands_aliases["blackjack"]]:
		if "none" in param[1] or param[
		    2] != "none":  # only bj <bet> ; nothing more than that 1 parameter
			color = discord_error_rgb_code
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Too few arguments given.\n\nUsage:\n`blackjack <amount or all>`",
			    color=color)
		...
		if bet != "all":
			try:
				...
				if bet < 100:
					...
					embed = discord.Embed(
					    description=
					    f"{emoji_error}  Invalid `<amount or all>` argument given. Bet must be at least 100.\n",
					    color=color)
					embed.set_author(name=username, icon_url=user_pfp)
					await channel.send(embed=embed)
					return
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  Invalid `<amount or all>` argument given.\n\nUsage:\n`roulette <amount or all> <space>`",
				    color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
		...
		try:
			...
			status, bj_return = await db_handler.blackjack(user, bet, client,
			                                               channel, username,
			                                               user_pfp, message)
	...
	# ATTENTION : for now roulette is only playable by ONE person, multiple can't play at once
	                 ...
	                 all_reg_commands_aliases["roulette"]]:  # no alias
		...
		if space not in ["odd", "even", "black", "red"]:
			...
			if fail == 1:
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  Invalid `<space>` argument given.\n\nUsage:\n`roulette <amount or all> <space>`",
				    color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
		...
		try:
			not_done = True
			# gotta check if enough money, if bet enough, etc etc then do the actual game
			status, roulette_return = await db_handler.roulette(
			    user, bet, space, client, channel, username, user_pfp, user_mention)
	...
	# --------------
		...
		try:
			status, slut_return = await db_handler.slut(user, channel, username,
			                                            user_pfp)
	...
	# --------------
		...
		try:
			status, crime_return = await db_handler.crime(user, channel, username,
			                                              user_pfp)
	...
	# --------------
		...
		try:
			status, work_return = await db_handler.work(user, channel, username,
			                                            user_pfp)
	...
	# --------------
		...
		try:
			status, rob_return = await db_handler.rob(user, channel, username,
			                                          user_pfp, user_to_rob)
	...
	# --------------
		...
		try:
			await db_handler.balance(user, channel, userbal_to_check,
			                         username_to_check, userpfp_to_check, not_done)
		except Exception as e:
			print(e)
			await send_error(channel)
	...
	# --------------
		...
		if "none" in param[1] or param[
		    2] != "none":  # we need 1 and only 1 parameter
			color = discord_error_rgb_code
			embed = discord.Embed(
			    description=
		...
		try:
			amount = str(amount)
			status, dep_return = await db_handler.deposit(user, channel, username,
			                                              user_pfp, amount)
	...
	# --------------
		...
		try:
			amount = str(amount)
			status, with_return = await db_handler.withdraw(user, channel, username,
			                                                user_pfp, amount)
	...
	# --------------
		...
		try:
			...
			if int(reception_user) == user:
				...
				embed = discord.Embed(
				    ...
				    f"(You may be looking for the `add-money` command.)",
				    color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Invalid `<member>` argument given.\n\nUsage:"
			    f"\n`give <member> <amount or all>`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
		...
		try:
			amount = str(amount)
			status, give_return = await db_handler.give(user, channel, username,
			                                            user_pfp, reception_user,
	...
	# --------------
		...
		# one argument
			...
			if param[1] in modes:
				...
				try:
					...
					embed = discord.Embed(
					    description=
					    f"{emoji_error}  Invalid `[-cash | -bank | -total]` argument given.\n\nUsage:\n"
					    f"`leaderboard [page] [-cash | -bank | -total]`",
					    color=color)
					embed.set_author(name=username, icon_url=user_pfp)
					await channel.send(embed=embed)
					return
		...
		print(
		    f"Looking for {full_name}, at page {page_number}, in mode {mode_type}")

		# handler

		try:
			status, lb_return = await db_handler.leaderboard(user, channel, username,
			                                                 full_name, page_number,
			                                                 mode_type, client)
	...
	# --------------
		...
		embed.add_field(
		    name=all_reg_commands[0],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[0]]}  |  "
		    f"Usage: `blackjack <amount or all>`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[1],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[1]]}  |  "
		    f"Usage: `roulette <amount or all> <space>`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[2],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[2]]}  |  "
		    f"Usage: `slut`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[3],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[3]]}  |  "
		    f"Usage: `crime`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[4],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[4]]}  |  "
		    f"Usage: `work`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[5],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[5]]}  |  "
		    f"Usage: `rob`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[6],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[6]]}  |  "
		    f"Usage: `balance`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[7],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[7]]}  |  "
		    f"Usage: `deposit <amount>`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[8],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[8]]}  |  "
		    f"Usage: `withdraw <amount>`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[9],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[9]]}  |  "
		    f"Usage: `give <member> <amount or all>`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[10],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[10]]}  |  "
		    f"Usage: `leaderboard [page] [-cash | -bank | -total]`",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[11],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[11]]}  |  "
		    f"Usage: `help` - shows this",
		    inline=False)
		embed.add_field(
		    name=all_reg_commands[12],
		    value=f"Alias: {all_reg_commands_aliases[all_reg_commands[12]]}  |  "
		    f"Usage: `module <module, e.g. slut>`",
		    inline=False)
		# edit stuff
		...
		embed.add_field(name="----------------------\n\nSTAFF ONLY",
		                value=f"requires <botmaster> role",
		                inline=False)
		embed.add_field(name="add-money",
		                value=f"Usage: `add-money <member> <amount>`",
		                inline=False)
		embed.add_field(
		    name="remove-money",
		    value=f"Usage: `remove-money <member> <amount> [cash/bank]`",
		    inline=False)
		embed.add_field(name="remove-money-role",
		                value=f"Usage: `remove-money-role <role> <amount>`",
		                inline=False)
		embed.add_field(name="change",
		                value=f"Usage: `change <module> <variable> <new value>`",
		                inline=False)
		embed.add_field(name="change-currency",
		                value=f"Usage: `change-currency <new emoji name>`",
		                inline=False)
		embed.add_field(name="set-income-reset",
		                value=f"Usage: `set-income-reset <false/true>`",
		                inline=False)
		embed.add_field(
		    name="remove-user-item",
		    value=f"Usage: `remove-user-item <member> <item short name> <amount>`",
		    inline=False)
		embed.add_field(
		    name="spawn-item",
		    value=f"Usage: `spawn-item <player pinged> <item short name> [amount]`",
		    inline=False)
		embed.add_field(name="clean-leaderboard",
		                value=f"Usage: `clean-leaderboard` - remove gone users",
		                inline=False)
		embed.add_field(name="----------------------\n\nITEM HANDLING",
		                value=f"create and delete requires <botmaster> role",
		                inline=False)
		embed.add_field(name="create-item",
		                value=f"Usage: `create-item`",
		                inline=False)
		embed.add_field(name="delete-item",
		                value=f"Usage: `delete-item <item short name>`",
		                inline=False)
		embed.add_field(name="buy-item",
		                value=f"Usage: `buy-item <item short name> <amount>`",
		                inline=False)
		embed.add_field(
		    name="give-item",
		    value=f"Usage: `give-item <member> <item short name> <amount>`",
		    inline=False)
		embed.add_field(name="use",
		                value=f"Usage: `use <item short name> <amount>`",
		                inline=False)
		embed.add_field(name="inventory",
		                value=f"Usage: `inventory [page]`",
		                inline=False)
		embed.add_field(name="user-inventory",
		                value=f"Usage: `user-inventory <member> [page]`",
		                inline=False)
		embed.add_field(name="catalog",
		                value=f"Usage: `catalog [item short name]`",
		                inline=False)
		embed.add_field(
		    name="----------------------\n\nINCOME ROLES",
		    value=f"create, delete and update requires <botmaster> role",
		    inline=False)
		embed.add_field(name="add-income-role",
		                value=f"Usage: `add-income-role <role pinged> <income>`",
		                inline=False)
		embed.add_field(name="remove-income-role",
		                value=f"Usage: `remove-income-role <role pinged>`",
		                inline=False)
		embed.add_field(name="list-roles",
		                value=f"Usage: `list-roles`",
		                inline=False)
		embed.add_field(
		    name="collect",
		    value=
		    f"Usage: `collect` | get your salary. If you choose to use update-income, please disable this command.",
		    inline=False)
		embed.add_field(
		    name="update-income",
		    value=
		    f"Usage: `update-income` | income works DAILY! automatically updates ALL INCOMES time elapsed * income.",
		    inline=False)
		# edit stuff
		embed.set_footer(text="For more info, contact an admin or <kendrik2.0>")
	...
	# --------------
		...
		if not staff_request:
			color = discord_error_rgb_code
			embed = discord.Embed(description=f"🔒 Requires botmaster role",
			                      color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			...
			if amount < 1:
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  Invalid `<amount>` argument given.\n\nUsage:\n`add-money <member> <amount>`",
				    color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
		...
		try:
			amount = str(amount)
			status, add_money_return = await db_handler.add_money(
			    user, channel, username, user_pfp, reception_user, amount,
			    reception_user_name)
	...
	# --------------
		...
		try:
			amount = str(amount)
			status, rm_money_return = await db_handler.remove_money(
			    user, channel, username, user_pfp, reception_user, amount,
			    reception_user_name, mode)
	...
	# --------------
		...
		if "none" in param[1] or "none" in param[2] or "none" in param[
		    3]:  # we need 3 parameters
			color = discord_error_rgb_code
			embed = discord.Embed(
			    description=
		...
		if param[2] == "name":
			color = discord_error_rgb_code
			embed = discord.Embed(
			    description=f"{emoji_error}  You cannot change module names.",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Invalid `<new value>` argument given.\n\nUsage:\n`change <module> <variable> <new value>`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			...
			status, edit_return = await db_handler.edit_variables(
			    user, channel, username, user_pfp, module_name, variable_name,
			    new_value)
	...
	# ---------------------------
		...
		try:
			status, emoji_edit_return = await db_handler.change_currency_symbol(
			    user, channel, username, user_pfp, new_emoji_name)
			if status == "error":
				color = discord_error_rgb_code
	...
	# ---------------------------
		...
		if param[1] not in ["true",
		                    "false"]:  # and that param has to be true/false
			color = discord_error_rgb_code
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Too few arguments given.\n\nUsage: `set-income-reset <false/true>`",
		...
		# handler
		try:
			status, new_income_reset_return = await db_handler.set_income_reset(
			    user, channel, username, user_pfp, new_income_reset)
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{new_income_reset_return}",
				                      color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
	...
	# ---------------------------
		...
		first_embed = discord.Embed(title="Item Info",
		...
		while currently_creating_item:
			...
			if checkpoints == 0:
				...
				first_embed.add_field(name="Display Name",
				                      value=f"{item_display_name}")
				first_embed.set_footer(text="Type cancel to quit")
			...
			if checkpoints == 1:
				...
				if len(item_name) > 10:
					await channel.send(
					    f"{emoji_error} The maximum length for an items short name is 10 characters. Please try again."
					)
					continue
				elif len(item_name) < 3:
					await channel.send(
					    f"{emoji_error}  The minimum length for an items short name is 3 characters. Please try again."
					)
					continue
				elif " " in item_name.strip():
					print(f"-{item_name}- -{item_name.strip()}")
					await channel.send(
					    f"{emoji_error}  short name has to be ONE word (dashes or underscores work)."
					)
					continue
				# good input
				...
				first_embed.set_footer(
				    text="Type cancel to quit or skip to skip this option")
				...
				first_embed.add_field(name="Description",
				                      value=f"{description}",
				                      inline=False)
				first_embed.set_footer(
				    text="Type cancel to quit or skip to skip this option")
				...
				first_embed.add_field(name="Time remaining",
				                      value=f"{duration} days left")
				first_embed.set_footer(
				    text="Type cancel to quit or skip to skip this option")
				...
				try:
					...
					if max_amount < 1:
						await channel.send(
						    f"{emoji_error}  Invalid max amount given. Please try again or type cancel to exit."
						)
						continue
				except:
					if user_input.lower() == "skip" or user_input.lower() == "infinity":
				...
				try:
					...
					for role_id in roles_clean_one:
						try:
							...
							await channel.send(
							    f"{emoji_error}  Invalid role given. Please try again.")
							raise NameError
				...
				try:
					...
					if max_bal < 1:
						await channel.send(
						    f"{emoji_error}  Invalid max balance given. Please try again or type cancel to exit."
						)
						continue
				except:
					if user_input.lower() == "skip":
				...
				first_embed.add_field(name="Reply message",
				                      value=f"{reply_message}",
				                      inline=False)
				first_embed.set_footer(
				    text="Type cancel to quit or skip to skip this option")
				...
				if user_input.lower() == "skip":
					...
					try:
						...
						await channel.send(
						    f"{emoji_error} URL not found. Please try again or skip.")
						continue
		...
		try:
			status, create_item_return = await db_handler.create_new_item(
			    item_display_name, item_name, cost, description, duration, stock,
			    max_amount, roles_id_required, roles_id_to_give, roles_id_to_remove,
	...
	# ---------------------------
		...
		sec_embed = discord.Embed(
		    title="Attention",
		    description=
		    "🚨 This will permanently delete the item, also for every user!\nDo you wish to continue? [y/N]",
		    color=discord.Color.from_rgb(3, 169, 244))
		sec_embed.set_footer(
		    text=
		    "Info: use remove-user-item to remove an item from a specific user.")
		...
		embed = discord.Embed(
		    description=
		    f"{emoji_worked}  Item has been removed from the store.\nNote: also deletes from everyone's inventory.",
		    color=color)
		embed.set_author(name=username, icon_url=user_pfp)
		await channel.send(embed=embed)
	...
	# ---------------------------
		...
		if "none" in param[1]:  # we need player pinged
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Too few arguments given.\n\nUsage:\n`remove-user-item <member> <item short name> <amount>`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			status, remove_user_item_return = await db_handler.remove_user_item(
			    user, channel, username, user_pfp, item_name, amount, player_ping,
			    reception_user_name)
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{remove_user_item_return}",
				                      color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
	...
	# ---------------------------
		...
		sec_embed = discord.Embed(
		    title="Attention",
		    description=
		    "🚨 This will permanently delete all user instances that left the server!\nDo you wish to continue? [y/N]",
		    color=discord.Color.from_rgb(3, 169, 244))
		await channel.send(embed=sec_embed)
		...
		embed = discord.Embed(
		    description=
		    f"{emoji_worked} {clean_lb_return} user(s) have been removed from database.",
		    color=color)
		embed.set_author(name=username, icon_url=user_pfp)
		await channel.send(embed=embed)
	...
	# ---------------------------
		...
		if "none" in param[1]:  # we need item name
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Too few arguments given.\n\nUsage:\n`buy-item <item short name> <amount>`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			status, buy_item_return = await db_handler.buy_item(
			    user, channel, username, user_pfp, item_name, amount, user_role_ids,
	...
	# ---------------------------
		...
		if "none" in param[1]:  # we need player pinged
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Too few arguments given.\n\nUsage:\n`give-item <player pinged> <item short name> <amount>`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			...
			if int(player_ping) == user:
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  You cannot trade items with yourself. That would be pointless...",
				    color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Invalid `<member>` argument given.\n\nUsage:"
			    f"\n`give-item <player pinged> <item> <amount>`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			status, give_item_return = await db_handler.give_item(
			    user, channel, username, user_pfp, item_name, amount, player_ping,
			    server, message.author, reception_user_name,
			    False)  # false is for spawn_mode = False
			if status == "error":
				color = discord_error_rgb_code
	...
	# ---------------------------
		...
		if "none" in param[1]:  # we need player pinged
			...
			embed = discord.Embed(
			    description=
			    f"{emoji_error}  Too few arguments given.\n\nUsage:\n`spawn-item <player pinged> <item short name> [amount]`",
			    color=color)
			embed.set_author(name=username, icon_url=user_pfp)
			await channel.send(embed=embed)
			return
		...
		try:
			status, give_item_return = await db_handler.give_item(
			    user, channel, username, user_pfp, item_name, amount, player_ping,
			    server, message.author, reception_user_name,
			    True)  # True is for spawn_mode = True
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{give_item_return}", color=color)
				embed.set_author(name=username, icon_url=user_pfp)
	...
	# --------------
		...
		if "none" in param[2]:  # we need item amount
			...
			try:
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  Amount must be a (whole) integer.\n\nUsage:\n`use-item <item short name> <amount>`",
				    color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
		...
		try:
			status, use_return = await db_handler.use_item(user, channel, username,
			                                               user_pfp, item_used,
			                                               amount_used)
	...
	# ---------------------------------------
		...
		if param[1] == "none":
			...
			try:
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  Invalid page number.\n\nUsage:\n`inventory [page]`",
				    color=color)
				embed.set_footer(
				    text=
				    "info: use it without page once, output will show amount of total pages.\ninfo: use user-inventory to see inventory of another user."
				)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
		...
		try:
			status, inventory_return = await db_handler.check_inventory(
			    user, channel, username, user_pfp, user_to_check,
	...
	# --------------------------------------------------------
		...
		if "none" in param[1]:  # we need a member pinged
			...
			try:
				...
				embed = discord.Embed(
				    description=
				    f"{emoji_error}  Invalid member ping.\n\nUsage:\n`user-inventory <member> [page]`",
				    color=color)
				embed.set_footer(
				    text="info: use `inventory` to see your own inventory.")
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
	...
	# ---------------------------
		...
		# handler
		try:
			status, catalog_return = await db_handler.catalog(
			    user, channel, username, user_pfp, item_check, server)
			if status == "error":
				color = discord_error_rgb_code
	...
	# ---------------------------
		...
		await channel.send(
		    "Info: the income amount specified is an DAILY one.\nRemember: you need to manually update income."
		)
		if not staff_request:
			color = discord_error_rgb_code
			embed = discord.Embed(description=f"🔒 Requires botmaster role",
		...
		# handler
		try:
			status, create_role_return = await db_handler.new_income_role(
	...
	# ---------------------------
	    ...
	    "remove-income-role", "delete-income-role", "remove-role-income",
	    "delete-role-income"
	]:
		if not staff_request:
			color = discord_error_rgb_code
		...
		income_role_beta = str(
		    param[1])  # see another instance where i use this to see why
		income_role = ""
		...
		# handler
		try:
			status, remove_role_return = await db_handler.remove_income_role(
			    user, channel, username, user_pfp, income_role)
			if status == "error":
				color = discord_error_rgb_code
		...
		color = discord.Color.from_rgb(102, 187, 106)  # green
		embed = discord.Embed(
		    description=f"{emoji_worked}  Role has been disabled as income role.",
		    color=color)
		embed.set_author(name=username, icon_url=user_pfp)
		await channel.send(embed=embed)
	...
	# ---------------------------
		...
		# handler
		try:
			status, remove_money_role_return = await db_handler.remove_money_role(
			    user, channel, username, user_pfp, server, income_role, amount)
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{remove_money_role_return}",
				                      color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
	...
	# ---------------------------
	    ...
	    "list-roles", "list-income-roles", "list-role-income", "list-incomes"
	]:
		try:
			status, list_roles_return = await db_handler.list_income_roles(
			    user, channel, username, user_pfp, server)
			if status == "error":
				color = discord_error_rgb_code
	...
	# ---------------------------
		...
		try:
			status, update_incomes_return = await db_handler.update_incomes(
			    user, channel, username, user_pfp, server)
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{update_incomes_return}",
				                      color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
		...
		embed = discord.Embed(
		    description=
		    f"{emoji_worked}  Users with registered roles have received their income (into bank account).",
		    color=color)
		embed.set_author(name=username, icon_url=user_pfp)
		await channel.send(embed=embed)
	...
	# ---------------------------
		...
		try:
			status, update_incomes_return = await db_handler.update_incomes_solo(
			    user, channel, username, user_pfp, server, user_roles)
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{update_incomes_return}",
				                      color=color)
	...
	# ---------------------------
		...
		try:
			status, economy_stats_return = await db_handler.economy_stats(
			    user, channel, username, user_pfp, server)
			if status == "error":
				color = discord_error_rgb_code
				embed = discord.Embed(description=f"{economy_stats_return}",
				                      color=color)
				embed.set_author(name=username, icon_url=user_pfp)
				await channel.send(embed=embed)
				return
