...
class blackjack_discord_implementation:
	...
	async def get_user_input(self, message):
		...
		answer = await self.bot.wait_for(
		    "message", check=lambda response: response.author == message.author)
	...
	async def play(self, bot, channel, username, user_pfp, message, bet):
		...
		while keepPlaying:
			...
			while playHuman:
				...
				if self.loopCount == 0:
					...
					firstEmbed = discord.Embed(
					    description=
					    f"Type `hit` to draw another card, or `stand` to pass.",
					    color=color)
					firstEmbed.set_author(name=username, icon_url=user_pfp)
					firstEmbed.add_field(
					    name="**Your hand**",
					    value=f"X X\nValue {self.handCount(hand['human'])}",
					    inline=True)
					firstEmbed.add_field(name="**Dealer shows**",
					                     value=f"{str(hand['computer'][-1])}\nValue ?",
					                     inline=True)
				...
				# which we will edit
					...
					newEmbed.add_field(
					    ...
					    value=f"X X\nValue {self.handCount(hand['human'])}",
					    inline=True)
					newEmbed.add_field(name="**Dealer shows**",
					                   value=f"{str(hand['computer'][-1])}\nValue ?",
					                   inline=True)
			...
			print("pc : ", self.handCount(hand['computer']), "player : ",
			      self.handCount(hand['human']))
			...
			if self.handCount(hand['human']) == 21:
				...
				pcBustedEmbed = discord.Embed(
				    description=
				    f"Result: blackjack! {str(self.currency_symbol)} +{'{:,}'.format(bet*1.5)}",
				    color=color)
				pcBustedEmbed.set_author(name=username, icon_url=user_pfp)
				pcBustedEmbed.add_field(
				    name="**Your hand**",
				    value=f"X X\nValue {self.handCount(hand['human'])}",
				    inline=True)
				pcBustedEmbed.add_field(
				    name="**Dealer shows**",
				    value=f"X X\nValue {self.handCount(hand['computer'])}",
				    inline=True)
				...
				playerBustEmbed = discord.Embed(
				    description=
				    f"Result: Bust {str(self.currency_symbol)} -{'{:,}'.format(bet)}",
				    color=color)
				playerBustEmbed.set_author(name=username, icon_url=user_pfp)
				playerBustEmbed.add_field(
				    name="**Your hand**",
				    value=f"X X\nValue {self.handCount(hand['human'])}",
				    inline=True)
				playerBustEmbed.add_field(
				...
				pcBustedEmbed = discord.Embed(
				    description=
				    f"Result: Dealer Bust {str(self.currency_symbol)} +{'{:,}'.format(bet)}",
				    color=color)
				pcBustedEmbed.set_author(name=username, icon_url=user_pfp)
				pcBustedEmbed.add_field(
				    name="**Your hand**",
				...
				color = discord.Color.from_rgb(102, 187, 106)
				playerWinEmbed = discord.Embed(
				    description=
				    f"Result: Win {str(self.currency_symbol)} +{'{:,}'.format(bet)}",
				    color=color)
				playerWinEmbed.set_author(name=username, icon_url=user_pfp)
				playerWinEmbed.add_field(
				    name="**Your hand**",
				    value=f"X X\nValue {self.handCount(hand['human'])}",
				    inline=True)
				playerWinEmbed.add_field(
				...
				playerWinEmbed = discord.Embed(description=f"Result: Push, money back",
				                               color=color)
				playerWinEmbed.set_author(name=username, icon_url=user_pfp)
				playerWinEmbed.add_field(
				    name="**Your hand**",
				...
				color = discord.Color.from_rgb(239, 83, 80)
				pcWinEmbed = discord.Embed(
				    description=
				    f"Result: Loss {str(self.currency_symbol)} -{'{:,}'.format(bet)}",
				    color=color)
				pcWinEmbed.set_author(name=username, icon_url=user_pfp)
				pcWinEmbed.add_field(
				    name="**Your hand**",
				    value=f"X X\nValue {self.handCount(hand['human'])}",
				    inline=True)
				pcWinEmbed.add_field(
